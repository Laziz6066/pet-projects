...
async def add_destination_channel(message: types.Message):
    try:
        ...
        await message.reply(
            "Пожалуйста, укажите корректный ID канала-получателя: /add_destination_channel -100321312890")
...
async def remove_destination_channel(message: types.Message):
    try:
        ...
        await message.reply(
            "Пожалуйста, укажите корректный ID канала-получателя: /remove_destination_channel -10013213127890")
...
async def set_channel_mapping(message: types.Message):
    ...
    if len(args) != 2:
        await message.reply(
            "Пожалуйста, укажите ID канала-источника и ID канала-получателя через пробел: /set_channel_mapping -1001234327890 -10003219876324321")
        return
    ...
    try:
        ...
        await message.reply(
            f"Канал {source_channel_id} теперь будет пересылать контент на канал {destination_channel_id}")
        save_channels()
...
async def send_last_messages_handler(message: types.Message):
    ...
    if len(args) == 2:
        try:
            ...
            await message.reply(
                "Пожалуйста, укажите корректные ID канала и количество сообщений: /last_messages -1001323290 5 или /last_messages -100321327832190 all")
            return
    elif len(args) == 1:
        try:
            if args[0].lower() == "all":
    ...
    for destination_channel_id in destination_channels:
        for message_group in grouped_messages.values():
            if len(message_group) > 1 and message_group[0].grouped_id:
                ...
                caption = "\n".join(
                    [replace_link(replace_at_word(msg.text, "@test"), new_link) for msg in message_group if msg.text])
